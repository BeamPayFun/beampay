...
- **抗黑名单手续费路径** —— 若手续费 leg 转给协议失败(如被 USDT/USDC 拉黑),手续费改派给订单的 `receiver`。不变量:`receiver 到账 + 协议到账 == 金额`,恒成立。
...
| 仓库 | 作用 | 技术栈 | 部署 |
|---|---|---|---|
| [beampay-contracts](https://github.com/BeamPayFun/beampay-contracts) | `BeamPayRouter` 合约 + 测试（[深度 README](https://github.com/BeamPayFun/beampay-contracts/blob/main/README.zh-CN.md)） | Foundry、solc 0.8.28、viaIR、Slither | Etherscan 验证 |
...
4. **费率变更经 7 天时间锁。**(其余治理操作为即时 `onlyGov` 调用,受本组不变量约束。)
5. **抗黑名单手续费路径** —— `receiver 到账 + 协议到账 == 金额`。
